...
# --- checkouts that vendor the guard shim and run nothing --------------------
# The mirror of the section below: that one is a hook wired to a script that is gone,
# this one a script that is present and wired to nothing. It is the quieter failure by
# far -- a missing script errors on every prompt, while an unwired guard is silence,
# and the trace it leaves is a task branch found stranded days later.


def _adopted(root, name: str, settings: object = "{}", shim: bool = False) -> None:
    """A checkout that vendors devkit, with `settings` written verbatim as text."""
    (root / name).mkdir(parents=True, exist_ok=True)
    (root / name / "DEVKIT_VERSION").write_text("abc1234\n", encoding="utf-8")
    if shim:
        launcher = root / name / "scripts" / "hooks" / "worktree-guard-launch.py"
        launcher.parent.mkdir(parents=True, exist_ok=True)
        launcher.write_text("# the shim\n", encoding="utf-8")
    path = root / name / ".claude" / "settings.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(settings if isinstance(settings, str) else json.dumps(settings))


def _guard_hook(command: str) -> dict:
    return {"hooks": {"PreToolUse": [{"hooks": [{"type": "command", "command": command}]}]}}


def test_a_checkout_that_adopted_devkit_and_runs_no_guard_is_named(tmp_path):
    _adopted(tmp_path, "carameli", shim=True)
    line = ws.unguarded_line(tmp_path, ["carameli"], source=REPO_ROOT)
    assert "carameli" in line
    assert "sync-devkit.py --pull" in line


def test_a_checkout_whose_vendored_copy_predates_the_shim_is_still_named(tmp_path):
    """The regression this line exists for. Requiring the shim on disk -- which is what
    `--check` does, since it reports a missing vendored file as drift -- made this
    silent for every checkout on the machine, because not one of them had pulled the
    release that added it. Missing file and missing hook are the same fault out here,
    and the same `--pull` closes both."""
    _adopted(tmp_path, "carameli", shim=False)
    assert "carameli" in ws.unguarded_line(tmp_path, ["carameli"], source=REPO_ROOT)


def test_a_checkout_that_wires_the_guard_is_silent(tmp_path):
    _adopted(
        tmp_path,
        "carameli",
        _guard_hook('python3 "${CLAUDE_PROJECT_DIR:-.}/scripts/hooks/worktree-guard-launch.py"'),
    )
    assert ws.unguarded_line(tmp_path, ["carameli"], source=REPO_ROOT) == ""


def test_a_repo_that_never_adopted_devkit_is_silent(tmp_path):
    """A repo not on devkit's harness has nothing to run the hook, and a standing
    demand to wire one is how a line stops being read."""
    (tmp_path / "notes" / ".claude").mkdir(parents=True)
    (tmp_path / "notes" / ".claude" / "settings.json").write_text("{}", encoding="utf-8")
    assert ws.unguarded_line(tmp_path, ["notes"], source=REPO_ROOT) == ""


def test_a_settings_file_that_cannot_be_read_is_silent(tmp_path):
    """Cannot-tell is not unguarded. The project may well be wired; reporting it on a
    parse failure is how a status line earns being ignored."""
    _adopted(tmp_path, "carameli", settings="{not json,")
    assert ws.unguarded_line(tmp_path, ["carameli"], source=REPO_ROOT) == ""


def test_every_unguarded_checkout_is_named_not_counted(tmp_path):
    _adopted(tmp_path, "carameli")
    _adopted(tmp_path, "ibkr_trader")
    line = ws.unguarded_line(tmp_path, ["carameli", "ibkr_trader"], source=REPO_ROOT)
    assert "carameli" in line
    assert "ibkr_trader" in line


def test_a_source_checkout_too_old_to_answer_is_silent_not_fatal(tmp_path):
    """`source` is the permanent devkit checkout, which can be older than the session's
    box. Nothing in this file may fail a session start, so a source with no such check
    is a shrug rather than a traceback."""
    _adopted(tmp_path, "carameli")
    assert ws.unguarded_line(tmp_path, ["carameli"], source=tmp_path / "nowhere") == ""
